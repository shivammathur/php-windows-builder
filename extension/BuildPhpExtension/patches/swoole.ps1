...
# Load Zend portability before Swoole's guarded Windows macros.
$replacement = '${1}#include "Zend/zend_portability.h"' + "`n" + '${2}'
(Get-Content src\coroutine\iocp.cc -Raw) -replace '(\*/\r?\n\r?\n)(#include "swoole_iocp.h")', $replacement | Set-Content src\coroutine\iocp.cc -NoNewline
